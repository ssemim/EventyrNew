            ...
            bcrypt.compare(req.body.password.toString(), data[0].password, (err,response) => {
                if(err) return res.json({Error: "패스워드 해독 에러 발생"});
                    if(response){
                        ...
                        const token = jwt.sign({id, name, position, oc_img, stat, heart, rank, point, badgeList, friends ,music, secretCount}, "jwt-secret-key-password-pst", {expiresIn:'8h'}) //가운데 문자열 비번 수정 + 세션 만료 시간 : 2시간
